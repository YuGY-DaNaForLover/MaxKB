...
from application_ext.serializers.application_ext_serializers import ApplicationExtSerializer, QaTextMapping
            ...
            for file in files:
                ...
                if QuerySet(Application).filter(name=f'{dsr_name}-{application_ext.get("title")}').exists():
                    ...
                    old_mapping_id_list = [old_mapping_list for old_mapping_list in QaTextMapping(instance=QuerySet(
                        ApplicationQaTextMapping).filter(application_id=application_id), many=True).data]
                    application_dict['qa_texts'] = self.to_application_qa_text_mapping(
                        application_qa_text_mapping_list=[*application_qa_text_mapping_list, *old_mapping_id_list], application_id=application_id, to_dict=True)
                    # print(json.dumps(application_dict))
