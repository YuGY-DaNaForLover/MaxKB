...
class XinfangSerializers(serializers.Serializer):

    class ApplicationUrlSerializer(serializers.Serializer):
        subject_identifier = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_("Subject Identifier")))

        def get_url(self, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            subject_identifier = self.data.get('subject_identifier')
            if not QuerySet(ApplicationExt).filter(subject_identifier=subject_identifier).exists():
                raise AppApiException(500, _('Application does not exist'))
            application_ext_model = QuerySet(ApplicationExt).filter(subject_identifier=subject_identifier).first()
            application_model = QuerySet(Application).filter(id=application_ext_model.application_id).first()
    ...
    class ApplicationCreateSerializer(serializers.Serializer):
        ...
        def create(self, with_valid=True):
            ...
            folder_path = os.path.join(
                CONFIG.get_default_dataset_mk_instance(), 'xinfang')
            if not os.path.exists(folder_path):
                raise AppApiException(500, '信访智能体会话模版不存在')
    ...
    class DatasetCreateSerializer(serializers.Serializer):
        ...
        source_url = serializers.CharField(
            required=True, error_messages=ErrMessage.char(_('web source url')), )
